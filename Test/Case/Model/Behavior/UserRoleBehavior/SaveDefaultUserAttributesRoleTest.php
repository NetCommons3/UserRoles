<?php
/**
 * UserRoleBehavior::saveDefaultUserAttributesRole()のテスト
 *
 ...
 */
class UserRoleBehaviorSaveDefaultUserAttributesRoleTest extends NetCommonsModelTestCase {
 ...
 */
	private function __expected($roleKey) {
		$expected = $this->TestModel->UserAttributesRole->find('all', array(
			'recursive' => -1,
			'conditions' => array('role_key' => $roleKey),
			'order' => array('id' => 'asc'),
		));
...
/**
 * saveDefaultUserAttributesRole()テストのDataProvider
 *
 * ### 戻り値
 *  - data User role data
 *
...
/**
 * saveDefaultUserAttributesRole()のテスト
 *
 * @param array $data User role data
 * @dataProvider dataProvider
 * @return void
 */
	public function testSaveDefaultUserAttributesRole($data) {
		//テスト実施
		$result = $this->TestModel->saveDefaultUserAttributesRole($data);
 ...
 */
	public function dataProviderSaveOnExceptionError() {
		...
		return array(
			array($data, 'UserRoles.UserAttributesRole', 'find'),
			array($data, 'UserRoles.UserAttributesRole', 'saveMany'),
		);
	}
 ...
 */
	public function testSaveOnExceptionError($data, $mockModel, $mockMethod) {
		...
		$this->TestModel->saveDefaultUserAttributesRole($data);
	}
