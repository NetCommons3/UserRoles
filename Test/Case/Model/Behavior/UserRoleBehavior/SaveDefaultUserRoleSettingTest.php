<?php
/**
 * UserRoleBehavior::saveDefaultUserRoleSetting()のテスト
 *
 ...
 */
class UserRoleBehaviorSaveDefaultUserRoleSettingTest extends NetCommonsModelTestCase {
 ...
 */
	private function __actual($roleKey) {
		$actual = $this->TestModel->UserRoleSetting->find('all', array(
			'recursive' => -1,
 ...
 */
	private function __expected($roleKey) {
		$expected = $this->TestModel->UserRoleSetting->find('all', array(
			'recursive' => -1,
			'conditions' => array('role_key' => $roleKey),
			'order' => array('id' => 'asc'),
		));
...
/**
 * saveDefaultUserRoleSetting()テストのDataProvider
 *
 * ### 戻り値
 *  - data User role data
 *
...
/**
 * saveDefaultUserRoleSetting()のテスト
 *
 * @param array $data User role data
 * @dataProvider dataProvider
 * @return void
 */
	public function testSaveDefaultUserRoleSetting($data) {
		//テスト実施
		$result = $this->TestModel->saveDefaultUserRoleSetting($data);
 ...
 */
	public function dataProviderSaveOnExceptionError() {
		...
		return array(
			array($data, 'UserRoles.UserRoleSetting', 'find'),
 ...
 */
	public function testSaveOnExceptionError($data, $mockModel, $mockMethod) {
		...
		$this->TestModel->saveDefaultUserRoleSetting($data);
	}
